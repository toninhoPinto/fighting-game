...
impl<'a> CommonAssets<'a> {
    ...
    pub fn load(texture_creator: &'a TextureCreator<WindowContext>) -> Self{
        ...
        let mut hit_sound = audio_player::load_from_file(Path::new("assets/sounds/104183__ekokubza123__punch.wav"))
        .map_err(|e| format!("Cannot load sound file: {:?}", e)).unwrap();
        let mut miss_sound = audio_player::load_from_file(Path::new("assets/sounds/60009__qubodup__swosh-22.wav"))
        .map_err(|e| format!("Cannot load sound file: {:?}", e)).unwrap();
        ...
        let hit2_anim: Vec<Texture> = asset_loader::load_anim_from_dir(
            &texture_creator,
            "assets/vfx/special_hit",
        );
        
        let mut vfx = HashMap::new();
        vfx.insert("normal_hit".to_string(), Animation::new(hit_anim, "normal_hit".to_string(), 0.35));
        vfx.insert("special_hit".to_string(), Animation::new(hit2_anim, "special_hit".to_string(), 0.35));
