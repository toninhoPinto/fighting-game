...
pub fn spawn_note(player: &Player, opponent: &Player, assets: &CharacterAssets, projectiles: &mut Vec<Projectile>) {
    ...
    if (player.animator.animation_index as f32 + 0.35 as f32) as usize >= p1_curr_anim as usize {
        ...
        if player.is_attacking && assets.effects.contains_key(&player_current_animation.name) {
            let mut projectile = (*assets.effects.get(&player_current_animation.name).unwrap()).clone();
            ...
            let target_pos = Point::new(opponent.position.x + (projectile.direction.x * 100), projectile.position.y);
